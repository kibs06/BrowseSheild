  ...
  useEffect(() => {
    ...
    const init = async () => {
      ...
      if (gradient && Array.isArray(gradient) && gradient.length >= 2) {
        ...
        gradient.forEach((entry, index) => grad.addColorStop(index / (gradient.length - 1), entry));
      ...
      const run = (timestamp) => {
        ...
        if (direction === 'horizontal') {
          for (let rowIndex = 0; rowIndex < tightHeight; rowIndex += 1) {
            const dx = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange);
            ctx.drawImage(offscreen, 0, rowIndex, offscreenWidth, 1, dx, rowIndex, offscreenWidth, 1);
          }
        } else if (direction === 'vertical') {
          for (let columnIndex = 0; columnIndex < offscreenWidth; columnIndex += 1) {
            const dy = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange);
            ctx.drawImage(offscreen, columnIndex, 0, 1, tightHeight, columnIndex, dy, 1, tightHeight);
          }
        } else {
          for (let rowIndex = 0; rowIndex < tightHeight; rowIndex += 1) {
            const dx = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange);
          ...
          for (let columnIndex = 0; columnIndex < offscreenWidth + fuzzRange; columnIndex += 1) {
            const dy = Math.floor(currentIntensity * (Math.random() - 0.5) * fuzzRange * 0.5);
            const columnData = ctx.getImageData(columnIndex, 0, 1, tightHeight + fuzzRange);
            ctx.clearRect(columnIndex, -fuzzRange, 1, tightHeight + 2 * fuzzRange);
            ctx.putImageData(columnData, columnIndex, dy);
          }
        }
